// 10x primitive: duolingo/completion-celebration v2
import SwiftUI
...
struct CompletionCelebrationConfig {
    ...
    /// Reference celebration headlines land in reward gold, not ink.
    var headlineColor: Color = PlayfulTokens.gold
    ...
    /// Session-end CTAs take the secondary blue in the reference app; the
    /// green stays on in-lesson CHECK/CONTINUE.
    var buttonConfig: DimensionalButtonConfig =
        DimensionalButtonConfig(face: PlayfulTokens.accentSecondary)
...
fileprivate struct CelebrationResultsStage: View {
    ...
    /// Reference stat-card anatomy: a saturated header band carrying the caps
    /// label in white, over a plain card body holding the tinted glyph +
    /// count-up value, all inside a solid tint border.
    private func statCard(_ stat: CompletionCelebrationStat, revealed: Bool) -> some View {
        let shape = RoundedRectangle(cornerRadius: 16, style: .continuous)
        return VStack(spacing: 0) {
            Text(stat.label.uppercased())
                .font(config.labelFont)
                .foregroundStyle(PlayfulTokens.inkOnAccent)
                .kerning(0.6)
                .lineLimit(1)
            ...
            .frame(maxWidth: .infinity)
            .padding(.vertical, 14)
            .background(config.cardFill)
        }
        .clipShape(shape)
        .overlay(shape.strokeBorder(stat.tint, lineWidth: 2.5))
    ...
    private func bonusRow(_ text: String) -> some View {
        Label(
            title: { Text(text).font(.system(.subheadline, design: .rounded, weight: .bold)) },
