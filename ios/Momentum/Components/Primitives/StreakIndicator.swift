...
struct StreakIndicatorConfig {
    ...
    /// Count numeral color. `nil` (default) matches the active tier's tint —
    /// the reference counter colors its number like its glyph.
    var countColor: Color? = nil
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            Text("\(count)")
                ...
                .foregroundStyle(count > 0 ? (config.countColor ?? tier?.tint ?? PlayfulTokens.ink)
                                           : config.inactiveTint)
                .monospacedDigit()
